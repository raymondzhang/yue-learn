...
const App = {
    ...
    el.innerHTML = `
      <div class="home-view">
        ...
        <div class="module-grid">
          <div class="module-card" onclick="App.navigateTo('characters')" style="--accent: #FF6B6B">
          ...
          <div class="module-card" onclick="App.navigateTo('cantonese')" style="--accent: #4ECDC4">
  ...
  renderCharacters(el) {
    ...
    el.innerHTML = `
      <div class="section-view">
        <button class="back-btn" onclick="App.navigateTo('home')">← 返回首页</button>
        <h2>📝 繁体字学堂</h2>
        ...
        <div class="category-grid">
          ${cats.map(cat => {
            ...
            const learned = cat.chars
              ...
              : 0;
            return `
            <div class="category-card" onclick="App.navigateTo('char-detail', {categoryId:'${cat.id}'})" style="--cat-color: ${this.getCategoryColor(cat.id)}">
  ...
  renderCharDetail(el, params) {
      ...
      content = `
        <div class="char-grid">
          ${cat.chars.map((c, i) => {
            ...
            return `
            <div class="char-card ${masteryClass}" onclick="App.showCharCard('${c.t}', ${i})" data-idx="${i}">
    ...
    el.innerHTML = `
      <div class="section-view">
        <button class="back-btn" onclick="App.navigateTo('characters')">← 返回分类</button>
        <h2>${cat.icon} ${cat.name}</h2>
        ${content}
        <div class="action-bar">
          <button class="btn btn-primary" onclick="App.startFlashcards(App.currentView === 'home' ? {} : {categoryId:'${cat.id}'})">
            🃏 开始闪卡学习
          </button>
          <button class="btn btn-secondary" onclick="App.startQuiz({type:'s2t', categoryId:'${cat.id}'})">
            ✏️ 做选择题
          </button>
        </div>
      </div>`;
  ...
  renderFlashcard(el) {
    ...
    if (!st || st.index >= st.total) {
      el.innerHTML = `
        <div class="section-view completion-view">
          ...
          <h2>太棒了！全部学完！</h2>
          <p>获得 ${st.total} 颗星星</p>
          <button class="btn btn-primary" onclick="App.navigateTo('home')">返回首页</button>
        </div>`;
      return;
    }
    ...
    el.innerHTML = `
      <div class="flashcard-view">
        <button class="back-btn" onclick="App.navigateTo('home')">← 返回</button>
        <div class="flashcard-progress">${st.index + 1} / ${st.total}</div>
        <div class="flashcard ${st.flipped ? 'flipped' : ''}" onclick="App.flipCard()">
        ...
        <div class="flashcard-nav">
          <button class="btn btn-secondary" onclick="App.flashcardPrev()">⬅ 上一个</button>
          <button class="btn btn-wrong" onclick="App.flashcardNext(false)">😕 不认识</button>
          <button class="btn btn-correct" onclick="App.flashcardNext(true)">✅ 认识</button>
        </div>
      </div>`;
    Speech.speakCantonese(c.t);
  },
    ...
    el.innerHTML = `
      <div class="section-view">
        ...
        <div class="scenario-grid">
          ${scenarios.map(s => `
            <div class="scenario-card" onclick="App.navigateTo('scenario', {scenarioId:'${s.id}'})">
        ...
        <div class="extra-sections">
          <div class="extra-card" onclick="App.navigateTo('comparison')">
            <div class="extra-icon">🔄</div>
            <div class="extra-name">粤语 vs 普通话</div>
            <div class="extra-desc">常用词对比</div>
          </div>
          <div class="extra-card" onclick="App.navigateTo('particles')">
            <div class="extra-icon">✨</div>
  ...
  renderScenario(el, params) {
    ...
    el.innerHTML = `
      <div class="section-view">
        <button class="back-btn" onclick="App.navigateTo('cantonese')">← 返回场景</button>
        <h2>${scenario.icon} ${scenario.name}</h2>
        <div class="phrase-list">
          ${scenario.phrases.map((p, i) => `
            <div class="phrase-item" data-idx="${i}">
              <div class="phrase-top" onclick="Speech.speakCantonese('${p.cantonese}'); App.recordPhraseListen('${scenario.id}', ${i})">
              ...
              <div class="phrase-details">
                ...
                <div class="phrase-actions-sm">
                  ...
                  <button class="btn-sm" onclick="App.togglePhraseAnswer(${i})">👀 看解释</button>
                </div>
  ...
  renderComparison(el) {
    el.innerHTML = `
      <div class="section-view">
        <button class="back-btn" onclick="App.navigateTo('cantonese')">← 返回</button>
        <h2>🔄 粤语 vs 普通话用词对比</h2>
      ...
      <div class="section-view">
        ...
        ${Progress.data.hearts <= 0 ? `
          <div class="no-hearts">
            ...
            <button class="btn btn-secondary" onclick="Progress.checkHeartRegen(); App.updateHeader(); App.navigateTo('home')">返回首页</button>
          </div>
        ` : `
          <div class="quiz-type-grid">
            <div class="quiz-type-card" onclick="App.startQuiz({type:'s2t'})">
              <div class="qt-icon">📝</div>
              <div class="qt-name">看简选繁</div>
              <div class="qt-desc">看简体字，选出正确的繁体字</div>
            </div>
            <div class="quiz-type-card" onclick="App.startQuiz({type:'t2s'})">
              <div class="qt-icon">🔤</div>
              <div class="qt-name">看繁选简</div>
              <div class="qt-desc">看繁体字，选出正确的简体字</div>
            </div>
            <div class="quiz-type-card" onclick="App.startQuiz({type:'listen'})">
              <div class="qt-icon">👂</div>
              <div class="qt-name">听音选字</div>
              <div class="qt-desc">听粤语发音，选出对应的字</div>
            </div>
            <div class="quiz-type-card" onclick="App.startQuiz({type:'phrase'})">
              <div class="qt-icon">💬</div>
              <div class="qt-name">粤语翻译</div>
              <div class="qt-desc">听粤语句子，选择普通话意思</div>
            </div>
            <div class="quiz-type-card" onclick="App.startQuiz({type:'fill'})">
              <div class="qt-icon">✏️</div>
              <div class="qt-name">选词填空</div>
              <div class="qt-desc">选择正确的粤语词填空</div>
            </div>
            <div class="quiz-type-card" onclick="App.startQuiz({type:'mix'})">
              <div class="qt-icon">🎲</div>
    ...
    el.innerHTML = `
      <div class="quiz-view">
        <div class="quiz-header">
          <button class="back-btn" onclick="App.navigateTo('quiz')">← 退出</button>
        ...
        <div class="quiz-body">
          ...
          <div class="quiz-options">
            ${q.options.map((opt, i) => `
              <div class="quiz-option" data-value="${opt}" onclick="App.answerQuiz(this, '${opt.replace(/'/g, "\\'")}')">
    ...
    el.innerHTML = `
      <div class="section-view completion-view">
        ...
        <div class="result-actions">
          <button class="btn btn-primary" onclick="App.startQuiz({type:'${qs.type}'})">再来一轮</button>
          <button class="btn btn-secondary" onclick="App.navigateTo('quiz')">换个题型</button>
  ...
  renderListening(el) {
    el.innerHTML = `
      <div class="section-view">
        ...
        <div class="quiz-type-grid">
          <div class="quiz-type-card" onclick="App.startListening({mode:'char'})">
            <div class="qt-icon">🔤</div>
            <div class="qt-name">听音辨字</div>
            <div class="qt-desc">听粤语发音，选出正确的字</div>
          </div>
          <div class="quiz-type-card" onclick="App.startListening({mode:'phrase'})">
            <div class="qt-icon">💬</div>
            <div class="qt-name">听句辨意</div>
            <div class="qt-desc">听粤语句子，选择普通话意思</div>
          </div>
          <div class="quiz-type-card" onclick="App.startListening({mode:'number'})">
            <div class="qt-icon">🔢</div>
            <div class="qt-name">粤语数字</div>
            <div class="qt-desc">听懂粤语数字1-100</div>
          </div>
          <div class="quiz-type-card" onclick="App.startListening({mode:'classroom'})">
            <div class="qt-icon">🏫</div>
  ...
  renderSpeaking(el) {
    el.innerHTML = `
      <div class="section-view">
        ...
        ` : ''}
        <div class="quiz-type-grid">
          <div class="quiz-type-card" onclick="App.startSpeaking({mode:'word'})">
            <div class="qt-icon">📝</div>
            <div class="qt-name">词语跟读</div>
            <div class="qt-desc">听发音，跟读单个词语</div>
          </div>
          <div class="quiz-type-card" onclick="App.startSpeaking({mode:'sentence'})">
            <div class="qt-icon">💬</div>
  ...
  renderSpeakItem(el) {
    ...
    if (!ss || ss.index >= ss.items.length) {
      ...
      el.innerHTML = `
        <div class="section-view completion-view">
          ...
          <button class="btn btn-primary" onclick="App.navigateTo('speaking')">再来一组</button>
          <button class="btn btn-secondary" onclick="App.navigateTo('home')">返回首页</button>
        </div>`;
      return;
    }
    ...
    el.innerHTML = `
      <div class="speak-view">
        <button class="back-btn" onclick="App.navigateTo('speaking')">← 返回</button>
        ...
        ${Speech.hasRecognition() ? `
          <div class="speak-record-section">
            <button class="btn btn-record" id="btn-record" onclick="App.startRecord()">
              🎤 点击跟读
            </button>
        ...
        ` : `
          <div class="self-eval">
            ...
            <div class="self-eval-btns">
              <button class="btn btn-correct" onclick="App.selfEval(true)">👍 读得不错</button>
              <button class="btn btn-wrong" onclick="App.selfEval(false)">😅 还需练习</button>
            </div>
          </div>
        `}
      </div>`;
  ...
  _renderVoiceStatus() {
    ...
    return `
      <div class="voice-status ${statusClass}">
        ...
        <button class="btn-sm voice-test-btn" onclick="event.stopPropagation(); App.navigateTo('voice-test')">🔊 ${info.hasVoice ? '测试语音' : '安装引导'}</button>
      </div>`;
  },
    ...
    el.innerHTML = `
      <div class="section-view">
        ...
        <div style="margin: 16px 0; display: flex; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn-primary" onclick="App.refreshVoices()">🔄 刷新语音列表</button>
        </div>
          ...
          ${zhVoices.length === 0 ? '<p style="padding:16px;">未找到任何中文语音</p>' : zhVoices.map((v, i) => `
            <div class="voice-item">
              ...
              <div style="display:flex;gap:6px;">
                <button class="btn-sm" onclick="App.testVoiceByIndex(${Speech.voices.indexOf(v) >= 0 ? Speech.voices.indexOf(v) : i}, '你好早晨食飯多謝返學')">试听</button>
                <button class="btn-sm" onclick="App.setAsCantonese(${Speech.voices.indexOf(v) >= 0 ? Speech.voices.indexOf(v) : i})">设为粤语</button>
              </div>
            </div>
          `).join('')}
        </div>
    ...
    el.innerHTML = `
      <div class="section-view">
        ...
        <div class="stats-actions">
          <button class="btn btn-danger" onclick="if(confirm('确定要清除所有学习记录吗？')) { Progress.resetAll(); App.updateHeader(); App.navigateTo('home'); }">
            🗑️ 重置所有进度
          </button>
        </div>
      </div>`;
